...
test_that ('loading JSON version 6 works', {
    log_threshold (WARN)
    test_data <- load_file_json (rren_example ('results-small-version-6.json'))
    expect_renaissance (test_data)
    expect_tibble (test_data, nrows = 6)
    expect_setequal (test_data |> distinct (benchmark) |> pull (benchmark), c ('scrabble', 'dotty'))
    expect_equal (test_data |> filter (benchmark == 'scrabble', index == 1) |> pull (time), 0.341145609)
})
...
test_that ('loading multiple JSON files works', {
    ...
    expect_renaissance (test_data)
    expect_tibble (test_data, nrows = 24)
    expect_setequal (test_data |> distinct (benchmark) |> pull (benchmark), c ('page-rank', 'naive-bayes', 'scrabble', 'dotty'))
    ...
    expect_equal (test_data |> filter (str_ends (run, 'results-small-version-6\\.json'), benchmark == 'scrabble', index == 1) |> pull (time), 0.341145609)
})
